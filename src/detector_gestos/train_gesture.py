...
def main():
    if tf is None:
        print("TensorFlow ausente. Instale com 'pip install tensorflow'.")
        return
    ...
    if not labels:
        print("Defina 'gesto_labels' no config.json.")
        return

    include_z = bool(cfg.get('gesto_include_z', False))
    mirror_x = bool(cfg.get('gesto_mirror_x', True))
    model_out = cfg.get('gesto_modelo_path') or os.path.join(root, 'models', 'gesture_keras.h5')
